...
#define CKNIFE_ADMIN_FLAG ADMFLAG_KICK
...
public void OnPluginStart() {
	for(int i = 0; i < sizeof(commands); i++) {
		RegAdminCmd(commands[i], Command_CKnife, CKNIFE_ADMIN_FLAG);
	}
...
void OpenCKnifeMenu(int client, char[] targetName = "") {
	...
	for(int i = 0; i < g_arAllKnives.Length; i++) {
		...
		CPrintToChat(client, "{white}You have %d seconds left to undo the Knife", knife.time - GetTime());
...
void RevertEverything(int admin, int userid) {
	...
	for(int i = 0; i < g_arAllKnives.Length; i++) {
		...
		FormatEx(message, sizeof(message), "{white}Canceling the Knife [{blue}%s {white}X {red}%s{white}] in progress...", knife.attackerName, knife.victimName);
		...
		if(g_cvSlayKnifer.BoolValue) {
			...
			if(knifer) {
				...
				FormatEx(message, sizeof(message), "{white}Successfully slayed {red}%s {white}for {olive}knifing.", knife.attackerName);
				PrintCKnifeMessage(message);
			}	
		}
		...
		if(g_cvKbanKnifer.BoolValue) {
			...
			if(knifer) {
				KR_BanClient(admin, knifer, g_cvKbanTime.IntValue, "Knifing");
			}
		}
		...
		if(zombie) {
			...
			FormatEx(message, sizeof(message), "{white}Successfully teleported {olive}%s {white}back to the old position", knife.victimName);
			PrintCKnifeMessage(message);
		}
		...
		for(int j = 0; j < knife.deadPeople.Length; j++) {
			...
			if(human) {
				...
				FormatEx(message, sizeof(message), "{white}Successfully turned {olive}%N {white}into {olive}human.", human);
				PrintCKnifeMessage(message);
			}
		}
	...
	if(!found) {
		CPrintToChat(admin, "{white}Looks like someone dealt with this knife!");
	}
}

void PrintCKnifeMessage(const char[] message) {
	if(g_cvPrintMessageType.IntValue == 0) { // all players
		CPrintToChatAll(message);
	} else { // admins only
		for(int i = 1; i <= MaxClients; i++) {
			if(!IsClientInGame(i) || IsFakeClient(i) || !CheckCommandAccess(i, "sm_admin", CKNIFE_ADMIN_FLAG, true)) {
				continue;
			}
